...
airline_data = cbind(airline_sentiment = airline$score,xdtm)
airline_data_tfidf = cbind(airline_sentiment = airline$score,xdtm_tfidf)

library(stringr)
str_replace_all(airline$airline_sentiment,c('neutral','positive','negative'),c('0',"1","-1"))
data1 = str_replace_all(airline$airline_sentiment,'neutral','0')
data2 = str_replace_all(data1,'positive','1')
data3 = str_replace_all(data2,'negative','-1')
score = as.numeric(data3)

airline = cbind(score,airline)
str(airline)
...
tree = rpart(sentiment_airline~.,train1)
rpart.plot(tree)
...
fmla <- formula("score ~ .") #formula object
